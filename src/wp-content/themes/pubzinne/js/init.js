...
jQuery( document ).ready(
	function() {
		...
		function pubzinne_resize_video(cont) {
			...
			cont.find( 'video' ).each( function() {
				...
				var video     = $self.addClass( 'pubzinne_resize' ).eq( 0 );
			...
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				...
				if ( $self.hasClass( 'trx_addons_resize' )
					...
					|| $self.addClass( 'pubzinne_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0
				) {
					return;
				}
	...
	function pubzinne_init_media_elements(cont) {
		var audio_selector = pubzinne_apply_filters( 'pubzinne_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
			video_selector = pubzinne_apply_filters( 'pubzinne_filter_mediaelements_video_selector', 'video:not(.inited):not([nocontrols]):not([controls="0"]):not([controls="false"]):not([controls="no"])' ),	//:not([autoplay])
			media_selector = audio_selector + ( audio_selector && video_selector ? ',' : '') + video_selector;
		if (PUBZINNE_STORAGE['use_mediaelements'] && cont.find( media_selector ).length > 0) {
			if ( window.mejs ) {
