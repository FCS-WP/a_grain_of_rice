	...
	if ($query->post_count > 0) {
		?><section class="related_wrap<?php if (!empty($trx_addons_args['class'])) echo ' '.esc_attr($trx_addons_args['class']); ?>">
			...
			if ( $trx_addons_args['slider'] ) {
                ...
                if ( isset($trx_addons_args['post_type']) && $trx_addons_args['post_type'] == 'cpt_services') {
                    $trx_addons_args['template'] = TRX_ADDONS_PLUGIN_CPT . 'services/tpl.light-item.php';
                    $trx_addons_args['featured'] = 'icon';
                    $trx_addons_args['featured_position'] = 'top';
