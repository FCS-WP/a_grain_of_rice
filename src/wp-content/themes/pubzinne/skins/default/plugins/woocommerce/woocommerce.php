...
if ( ! function_exists( 'pubzinne_woocommerce_theme_setup9' ) ) {
	...
	function pubzinne_woocommerce_theme_setup9() {
		if ( pubzinne_exists_woocommerce() ) {
            ...
            add_filter( 'woocommerce_product_related_products_heading', 'pubzinne_woocommerce_related_title' );
			if ( ! is_admin() ) {
	...
	function pubzinne_woocommerce_add_to_cart_text( $text = '' ) {
		...
		return is_object( $product ) && $product->is_in_stock() && $product->is_purchasable()
				...
				&& ( 'external' !== $product->get_type() || $product->get_button_text() == '' )
					? esc_html__( 'Add to Cart', 'pubzinne' )
					: $text;
	}
}
...
// Related Title
if ( ! function_exists( 'pubzinne_woocommerce_related_title' ) ) {
    function pubzinne_woocommerce_related_title( $heading ) {
        $heading = esc_html__( 'You may also like', 'pubzinne' );
        return $heading;
    }
}

// Display
if ( ! function_exists( 'pubzinne_woocommerce_template_loop_rating' ) ) {
    function pubzinne_woocommerce_template_loop_rating(){
        global $product;
        $cat = pubzinne_storage_get('in_product_category');
        if (empty($cat) || !is_object($cat)) {
            if (is_object($product)) {
...
if ( ! function_exists( 'pubzinne_woocommerce_get_product_search_form' ) ) {
	...
	function pubzinne_woocommerce_get_product_search_form( $form, $role = 'search' ) {
		return '
...
// Change price filter step
if ( ! function_exists( 'pubzinne_woocommerce_price_filter_widget_step' ) ) {
    add_filter('woocommerce_price_filter_widget_step', 'pubzinne_woocommerce_price_filter_widget_step');
    function pubzinne_woocommerce_price_filter_widget_step( $step = '' ) {
        $step = 1;
        return $step;
    }
}
